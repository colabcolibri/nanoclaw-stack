  ...
  return (
    <div className="flex flex-col gap-6 w-full flex-1">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-(--border-main) pb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1 text-xs text-(--text-dim) font-mono">
                            <span>ID: {task.id}</span>
                            <button
                              onClick={() => handleCopy(task.id, task.id)}
                              className="p-1 hover:text-(--text-main) cursor-pointer"
                              title="Copiar ID"
                            >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                      {task.recurrence && (
                        <div className="p-2.5 rounded-lg bg-(--bg-input) border border-(--border-main)">
                    ...
                    {/* Prompt Box */}
                    <div className="p-3.5 rounded-lg bg-(--bg-input) border border-(--border-main) text-xs">
                      ...
                      <p className={`text-(--text-main) leading-relaxed ${isExpanded ? 'whitespace-pre-wrap' : 'line-clamp-3'}`}>
                        {displayPrompt}
                      </p>
                    </div>
                  </div>
          ...
          >
            ...
            <div className="p-5 border-b border-(--border-main) bg-(--bg-card-subtle) flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-(--text-main)">Instrução Completa da Rotina</h3>
                  <p className="text-xs font-mono text-(--text-dim)">ID: {viewingTask.id}</p>
                </div>
              </div>
              <Button
                variant="ghost"
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs text-(--text-main) leading-relaxed">
              ...
              <div>
                <span className="text-[10px] font-bold text-(--text-dim) uppercase font-mono block mb-2">Texto Integral:</span>
                <div className="p-4 rounded-xl bg-(--bg-input) border border-(--border-main) whitespace-pre-wrap font-mono text-xs leading-relaxed text-(--text-main)">
      ...
      {editingTask && (
        ...
        >
          <div
            className="w-full max-w-2xl bg-(--bg-card) border border-(--border-main) rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] cursor-default animate-in zoom-in-95 duration-150"
            onClick={(e) => e.stopPropagation()}
          >
            <form onSubmit={handleSaveEdit} className="flex flex-col h-full">
              ...
              <div className="p-5 border-b border-(--border-main) bg-(--bg-card-subtle) flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-(--text-main)">Editar Rotina Recorrente</h3>
                    <p className="text-xs font-mono text-(--text-dim)">ID: {editingTask.id}</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
              ...
              <div className="p-6 overflow-y-auto space-y-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-(--bg-input) border border-(--border-main) rounded-lg text-xs font-mono font-bold text-(--text-input) focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
                    placeholder="0 * * * *"
                  />
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    <span className="text-[10px] text-(--text-dim) font-bold">Atalhos rápidos:</span>
                    <button
                      type="button"
                      onClick={() => setEditCron('0 * * * *')}
                      className="px-2 py-0.5 rounded bg-(--bg-input) border border-(--border-main) text-[10px] font-mono hover:border-sky-500 cursor-pointer"
                    >
                      A cada 1h (0 * * * *)
                    </button>
                    <button
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-(--bg-input) border border-(--border-main) rounded-lg text-xs leading-relaxed text-(--text-input) focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 font-mono"
              ...
              {/* Footer */}
              <div className="p-4 border-t border-(--border-main) bg-(--bg-card-subtle) flex justify-end gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  type="button"
      ...
      {deletingTask && (
        ...
        >
          <div
            className="w-full max-w-md bg-(--bg-card) border border-red-500/30 rounded-2xl shadow-2xl overflow-hidden p-6 space-y-4 cursor-default animate-in zoom-in-95 duration-150 text-(--text-main)"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="text-center space-y-1">
              <h3 className="text-base font-bold text-(--text-main)">Cancelar e Excluir Rotina?</h3>
              <p className="text-xs text-(--text-muted) leading-relaxed">
                Você tem certeza que deseja cancelar a rotina <strong className="font-mono text-(--text-main)">{deletingTask.id}</strong>? O sistema não executará mais este agendamento.
              </p>
            </div>
