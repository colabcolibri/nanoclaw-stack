...
echo "🚀 Iniciando compilação do Barão macOS v1.3.1..."
...
# 3. Gerar arquivo ZIP
echo "🗜️ Gerando arquivo compactado Barao-macOS-v1.3.1.zip..."
cd "$SCRIPT_DIR/dist"
zip -r -q "Barao-macOS-v1.3.1.zip" "$BUNDLE_NAME"
cp "Barao-macOS-v1.3.1.zip" "Barao-macOS.zip"
...
if command -v hdiutil >/dev/null 2>&1; then
    ...
    rm -rf "$DMG_TMP" "$SCRIPT_DIR/dist/Barao.dmg" "$SCRIPT_DIR/dist/Barao-v1.3.1.dmg"
    mkdir -p "$DMG_TMP"
    cp -R "$APP_DIR" "$DMG_TMP/"
    ln -s /Applications "$DMG_TMP/Applications"
    
    hdiutil create -volname "Barao AI Installer" -srcfolder "$DMG_TMP" -ov -format UDZO "$SCRIPT_DIR/dist/Barao.dmg"
    cp "$SCRIPT_DIR/dist/Barao.dmg" "$SCRIPT_DIR/dist/Barao-v1.3.1.dmg"
    rm -rf "$DMG_TMP"
    echo "✅ Instalador DMG gerado com sucesso!"
fi

echo "=========================================="
echo "🎉 Versão v1.3.1 compilada com sucesso!"
