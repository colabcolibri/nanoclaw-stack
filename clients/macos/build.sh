...
# 3. Gerar arquivo ZIP
echo "🗜️ Gerando arquivo compactado Barao-macOS-v${APP_VERSION}.zip..."
cd "$SCRIPT_DIR/dist"
zip -r -q "Barao-macOS-v${APP_VERSION}.zip" "$BUNDLE_NAME"
cp "Barao-macOS-v${APP_VERSION}.zip" "Barao-macOS.zip"
...
if command -v hdiutil >/dev/null 2>&1; then
    ...
    rm -rf "$DMG_TMP" "$SCRIPT_DIR/dist/Barao.dmg" "$SCRIPT_DIR/dist/Barao-v${APP_VERSION}.dmg"
    mkdir -p "$DMG_TMP"
    cp -R "$APP_DIR" "$DMG_TMP/"
    ln -s /Applications "$DMG_TMP/Applications"
    
    hdiutil create -volname "Barao AI Installer" -srcfolder "$DMG_TMP" -ov -format UDZO "$SCRIPT_DIR/dist/Barao.dmg"
    cp "$SCRIPT_DIR/dist/Barao.dmg" "$SCRIPT_DIR/dist/Barao-v${APP_VERSION}.dmg"
    rm -rf "$DMG_TMP"
    echo "✅ Instalador DMG gerado com sucesso!"
fi

echo "=========================================="
echo "🎉 Versão v${APP_VERSION} compilada com sucesso!"
