/**
 * Sync-turn implementation for macOS / UI proxy channels.
 * Uses agent-runner orchestrator (Bun) + host conversation DB (better-sqlite3).
 * Invoked in-process under Bun, or via scripts/sync-turn-runner.ts from Node.
 */
import fs from 'fs';
...
export interface SyncTurnResult {
  ...
  /** Active session after this turn — authoritative for clients. */
  sessionId: string;
}
