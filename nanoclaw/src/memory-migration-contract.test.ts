...
const migrateSkillPath = path.resolve('.claude/skills/migrate-memory/SKILL.md');
const updateSkillPath = path.resolve('.claude/skills/update-nanoclaw/SKILL.md');

if (fs.existsSync(migrateSkillPath)) {
  describe('shared-memory migration contract', () => {
    const skill = fs.readFileSync(migrateSkillPath, 'utf-8');
...
if (fs.existsSync(updateSkillPath)) {
  describe('update-nanoclaw migration contract', () => {
    const updateSkill = fs.readFileSync(updateSkillPath, 'utf-8');
