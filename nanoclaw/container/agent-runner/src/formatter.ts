...
const ADMIN_COMMANDS = new Set(['/remote-control', '/clear', '/compact', '/context', '/cost', '/files', '/upload-trace', '/new', '/new-resume']);
 ...
 *
 * Prepends a `<context timezone="<IANA>" [city="…"] [country="…"] />` header so
 * the agent always knows what timezone it's in and where the user is based —
 * every timestamp it sees in message bodies is the user's local time, and every
 * time it produces (schedules, suggests) should be interpreted as local time in
 * that same zone. This header is v1 behavior (src/v1/router.ts:20-22); dropping
