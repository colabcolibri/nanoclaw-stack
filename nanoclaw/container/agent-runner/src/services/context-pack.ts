...
export class ContextPack {
  static formatMemoIndex(limit = 8): string {
    ...
    if (memos.length === 0) return '(nenhum memo recente)';
  ...
  static formatMemosSection(memos: MessageMemoEntry[]): string {
    ...
    return `## Contexto recente (memos)\n${lines.join('\n')}\n\nUse apenas o que for necessário para responder.`;
  }
  ...
  static buildMemorySection(cwd: string, plan: ContextPlan | undefined): string {
    ...
    if (!index) return '';
    return `## Índice de memória de longo prazo\n${index}`;
  }
}
